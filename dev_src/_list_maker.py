...
from pyroboxCore import config, logger, SimpleHTTPRequestHandler as SH_base
...
def list_directory_json(self:SH_base, path=None):
...
def list_directory(self:SH_base, path, user:User):
	...
	if user.NOPERMISSION:
		return self.send_error(HTTPStatus.FORBIDDEN, "Sorry, You don't have permission to view Server Content")
